...
We want to check for what periode of time there is electrical generation data for each country before performing other EDA.

```{r}
energy_summary_grouped <- energy_summary |>
  mutate(time_range = paste(min_year, max_year, sep='-'))
```
...
Not all the countries have data for the last 39 years. the @fig-electricity-eneration-data-time-span graph provides the span for each category. This will help to better understand trend later in our analysis.
